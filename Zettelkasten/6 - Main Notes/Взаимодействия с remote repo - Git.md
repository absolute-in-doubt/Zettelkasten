...
- `git fetch` - извлекаем изменения из remote repo в отдельную ветку `origin/main`или  `origin/feature-x`
Как посмотреть, что именно пришло:

- `git branch -r` — список удалённых веток (`origin/...`);
    
- `git log origin/main` — новые коммиты относительно твоей ветки;
    
- `git diff main origin/main` — разница между локальной и удалённой веткой;
    
- при желании можно даже временно перейти в удалённую ветку: `git checkout origin/main` (в read-only режиме через detached HEAD).

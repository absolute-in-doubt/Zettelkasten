...
---
### Односторонняя и двухсторонняя проверка:
...
Т.к. мы проверяем, что коэффициент ИЛИ больше нуля ИЛИ меньше.

Сравниваем:
### $t > t_{\frac{\alpha}{2}, df} \to H_0 \text{ отвергается }$ 
...
> [!note] 
> Тут будет точно такая же формула t-stat:
> 
> ### $t = \frac{\tilde a_i - 0}{S_{a_i}}$
> 
> Но, т.к. альтернатива односторонняя ($a_i > 0$):
> ### $t > t_{\alpha, df} \to H_o \text{ отвергается}$

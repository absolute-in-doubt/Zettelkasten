...
BinaryTree обычно строится с помощью метода `compareTo(Object obj)` интерфейса `Comparable`. Но `HashMap` не требует, чтобы ключи реализовывали `Comparble`. Поэтому алгоритм балансировки дерева такой:

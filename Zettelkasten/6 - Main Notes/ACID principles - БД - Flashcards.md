...
- **Read commited** (реализуется через MVCC - версионирование изменений)- читаются только зафиксированные изменения. - **Postgres standard**
- **Repeatable read** - все данные, прочитанные транзакцией, гарантированно не изменятся другими транзакциями до её завершения. (**НЕ фиксит lost update#2 - на картинке не верно**)
- **Serializable** - полная изоляция: транзакции выполняются так, как будто они идут последовательно.
	
`SELECT FOR UPDATE/FOR SHARE` не привязаны к конкретному уровню изоляции и их использование необходимо для предотвращения lost update на всех уровнях изоляции кроме Serializable.
...
Q: Какие аномалии работы с данными предотвращают pessimistic/optimistic locks? Как они связаны с уровнями изоляции?
A:  Они предотвращают lost update второго типа (первого типа - предотвращается самой БД).

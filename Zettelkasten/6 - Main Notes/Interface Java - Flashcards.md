...
Q: Может ли интерфейс содержать не static поля?
...
A: Вложенные интерфейсы могут иметь любой модификатор доступа. Вложенные интерфейсы всегда static (т.к. нет смысла ебаться с объектом внешнего класса, мы не можем на него ссылаться)
<!--ID: 1757698235430-->
...
Q: Можно ли использовать `this` в default методах интерфейса? Если да, то на кого они указывают, на интерфейс или на реализацию?
A: В default-методе интерфейса `this` можно использовать, и он указывает на **конкретный** экземпляр класса, который реализует интерфейс, а не на интерфейс как таковой.
<!--ID: 1768462479360-->

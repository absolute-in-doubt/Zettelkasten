...
CREATE TABLE Student_id_cards (
    ...
    student_id UNIQUE REFERENCES Students(id) 
);
```
A: Это связь One to One. Однако, использование двух id здесь излишне. Такая реализация применяется в случае, если PRIMARY KEY таблицы с уникальными реальными (не искусственно созданными) данными является конфиденциальным и швырять его по всему приложению - не варик. (в нашем случае registration_number - конфиденциален).

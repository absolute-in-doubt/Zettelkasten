...
Q: Могут ли использоваться агрегатные функции без GROUP BY в SELECT запросах?
A: Да. В таком случае агрегатная функция считается по всему набору строк, который вернул FROM/WHERE.
	
А также, это значит, что возвратить такой SELECT может только результаты агрегатных функций и кончтанты. (Поскольку нет полей, по которым группируем).
<!--ID: 1769261443912-->
